...
def make_posegraph_for_fragment(path_dataset, sid, eid, color_files, depth_files, fragment_id, n_fragments, intrinsic, with_opencv, config, stop_event):
    ...
    for s in range(sid, eid):
        for t in range(s + 1, eid):
            if stop_event.is_set():
                print(f"Stopping posegraph creation for fragment {fragment_id}")
                return
            if t == s + 1:
                print(f"Fragment {fragment_id:03d} / {n_fragments - 1:03d} :: RGBD matching between frame : {s} and {t}")
...
def integrate_rgb_frames_for_fragment(color_files, depth_files, fragment_id, n_fragments, pose_graph_name, intrinsic, config, stop_event):
    ...
    for i in range(len(pose_graph.nodes)):
        if stop_event.is_set():
            print(f"Stopping integration for fragment {fragment_id}")
            return
        i_abs = fragment_id * config['n_frames_per_fragment'] + i
        print(f"Fragment {fragment_id:03d} / {n_fragments - 1:03d} :: integrate rgbd frame {i_abs} ({i + 1} of {len(pose_graph.nodes)}).")
...
def make_pointcloud_for_fragment(path_dataset, color_files, depth_files, fragment_id, n_fragments, intrinsic, config, stop_event):
    mesh = integrate_rgb_frames_for_fragment(color_files, depth_files, fragment_id, n_fragments, join(path_dataset, config["template_fragment_posegraph_optimized"] % fragment_id), intrinsic, config, stop_event)
...
def process_single_fragment(fragment_id, color_files, depth_files, n_files, n_fragments, config, stop_event):
    if stop_event.is_set():
        print(f"Skipping fragment {fragment_id} as stop event is set")
        return
    ...
    make_posegraph_for_fragment(config["path_dataset"], sid, eid, color_files, depth_files, fragment_id, n_fragments, intrinsic, with_opencv, config, stop_event)
    if stop_event.is_set():
        return
    optimize_posegraph_for_fragment(config["path_dataset"], fragment_id, config)
    if stop_event.is_set():
        return
    make_pointcloud_for_fragment(config["path_dataset"], color_files, depth_files, fragment_id, n_fragments, intrinsic, config, stop_event)
...
def run(config, stop_event):
    ...
    if config["python_multi_threading"] is True:
        ...
        with mp_context.Pool(processes=max_workers) as pool:
            args = [(fragment_id, color_files, depth_files, n_files, n_fragments, config, stop_event) for fragment_id in range(n_fragments)]
            pool.starmap(process_single_fragment, args)
    else:
        for fragment_id in range(n_fragments):
            process_single_fragment(fragment_id, color_files, depth_files, n_files, n_fragments, config, stop_event)
