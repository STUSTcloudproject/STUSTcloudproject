...
class AppWindow:
    ...
    MENU_QUIT = 3
    MENU_VIEW_CONTROLS = 11
    MENU_POINT_CLOUD_CONTROLS = 12
    MENU_STATUS_PANEL = 14
    MENU_ABOUT = 21
    MENU_RESET_CAMERA = 31
    MENU_UNDO = 40
    MENU_REDO = 41
    MENU_NEXT_SOURCE = 42
    MENU_SWITCH_TARGET = 43
    MENU_START_REGISTRATION = 50
    MENU_MERGE_POINT_CLOUDS = 51
    MENU_CANCEL_MERGE = 52
    ...
    def __init__(self, width, height):
        ...
        # 更新菜單欄中的勾選狀態
        gui.Application.instance.menubar.set_checked(AppWindow.MENU_VIEW_CONTROLS, self.view_ctrls.visible)
        gui.Application.instance.menubar.set_checked(AppWindow.MENU_POINT_CLOUD_CONTROLS, self.point_cloud_ctrls.visible)
    ...
    def _setup_menu_bar(self):
        ...
        if gui.Application.instance.menubar is None:
            ...
            view_menu = gui.Menu()
            view_menu.add_item("Reset Camera\t\tC", AppWindow.MENU_RESET_CAMERA)
            edit_menu = gui.Menu()
            edit_menu.add_item("Undo\t\tZ", AppWindow.MENU_UNDO)
            edit_menu.add_item("Redo\t\tY", AppWindow.MENU_REDO)
            edit_menu.add_item("Next Source\t\tN", AppWindow.MENU_NEXT_SOURCE)
            edit_menu.add_item("Switch Target\t\tB", AppWindow.MENU_SWITCH_TARGET)
            process_menu = gui.Menu()
            process_menu.add_item("Start Registration\t\tR", AppWindow.MENU_START_REGISTRATION)
            process_menu.add_item("Merge Point Clouds\t\tM", AppWindow.MENU_MERGE_POINT_CLOUDS)
            process_menu.add_item("Cancel Merge\t\tP", AppWindow.MENU_CANCEL_MERGE)
            settings_menu = gui.Menu()
            settings_menu.add_item("View Controls", AppWindow.MENU_VIEW_CONTROLS)
            settings_menu.add_item("Point Cloud Controls", AppWindow.MENU_POINT_CLOUD_CONTROLS)
            ...
            if isMacOS:
                ...
                menu.add_menu("Process", process_menu)
        ...
        self.window.set_on_menu_item_activated(AppWindow.MENU_RESET_CAMERA, self._on_menu_reset_camera)
        ...
        self.window.set_on_menu_item_activated(AppWindow.MENU_NEXT_SOURCE, self._on_menu_next_source)
        self.window.set_on_menu_item_activated(AppWindow.MENU_SWITCH_TARGET, self._on_menu_switch_target)
        self.window.set_on_menu_item_activated(AppWindow.MENU_START_REGISTRATION, self._on_menu_start_registration)
        self.window.set_on_menu_item_activated(AppWindow.MENU_MERGE_POINT_CLOUDS, self._on_menu_merge_point_clouds)
        self.window.set_on_menu_item_activated(AppWindow.MENU_CANCEL_MERGE, self._on_menu_cancel_merge)
        self.window.set_on_menu_item_activated(AppWindow.MENU_VIEW_CONTROLS, self._on_menu_toggle_view_controls)
        self.window.set_on_menu_item_activated(AppWindow.MENU_POINT_CLOUD_CONTROLS, self._on_menu_toggle_point_cloud_ctrls)
    ...
    def _setup_translation_rotation_sliders(self):
        ...
        self.point_cloud_ctrls.add_child(gui.Label("Translation Step"))
        self.point_cloud_ctrls.add_child(self._translation_step_slider)
        ...
        self.point_cloud_ctrls.add_child(gui.Label("Rotation Step"))
        self.point_cloud_ctrls.add_child(self._rotation_step_slider)
    ...
    def _on_menu_reset_camera(self):
    ...
    def _on_menu_next_source(self):
        self._next_source()

    def _on_menu_switch_target(self):
        self._switch_target()

    def _on_menu_start_registration(self):
        self._on_start_registration()

    def _on_menu_merge_point_clouds(self):
        self._merge_point_clouds()

    def _on_menu_cancel_merge(self):
        self._cancel_merge()
    ...
    def _on_menu_toggle_point_cloud_ctrls(self):
        """
        點擊菜單中的“Point Cloud Controls”選項時觸發
        """
    ...
    def _update_settings_panel_visibility(self):
        ...
        更新設置面板的可見性
        """
        any_visible = self.view_ctrls.visible or self.point_cloud_ctrls.visible or self.status_panel.visible
