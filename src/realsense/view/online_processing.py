...
class PipelineModel:
    ...
    def __init__(self,
        ...
        self.process = subprocess.Popen(
            ['python', f'{os.path.dirname(os.path.abspath(__file__))}\\online_processing_pc_vis.py'],
        ...
        threading.Thread(target=read_process_output, args=(self.process,), daemon=True).start()
    ...
    def close_subprocess(self):
        ...
        try:
            ...
            print(stdout)  # 或者 log 到文件
            print(stderr)  # 或者 log 到文件
    ...
    def save_pcd(self):
        ...
        # Check if the subprocess is still running and stdin is writable
        if self.process.poll() is None:  # Check if the subprocess is still running
            try:
                self.process.stdin.write(f"{filename}\n")
                self.process.stdin.flush()
            except (BrokenPipeError, OSError) as e:
                log.error(f"Failed to write to subprocess stdin: {e}")
    ...
    def rotate_pcd_180_x(self):
        """Rotate the point cloud along the X axis by 180 degrees."""
        R = np.eye(3, dtype=np.float32)  # Ensure the rotation matrix is Float32
        R[1, 1], R[1, 2] = -1, 0
        R[2, 1], R[2, 2] = 0, -1
        
        # Convert point positions to Float32 if they are not already
...
class PipelineController:
    ...
    def on_window_close(self):
        ...
        self.pipeline_model.close_subprocess()
